...
class PaddleOcrProcessor(private val context: Context) {
    ...
    suspend fun recognize(bitmap: Bitmap): String = withContext(Dispatchers.IO) {
        ...
        ocr.runSync(bitmap).fold(
            onSuccess = { result ->
                val raw = result.rawOCRResult
                if (raw != null && raw.mText != null && raw.mText.isNotEmpty()) {
                    val first = raw.mText[0] ?: ""
                    android.util.Log.d(TAG, "mText[0] length=${first.length}, chars=${first.map { it.code }}")
                    android.util.Log.d(TAG, "mText size=${raw.mText.size}, all=${raw.mText.toList()}")
                    // 每个元素去掉内部换行后作为一行
                    val lines = raw.mText
                        .mapNotNull { it?.replace("\n", "")?.replace("\r", "")?.trim()?.takeIf { s -> s.isNotBlank() } }
                    android.util.Log.d(TAG, "OCR lines (${lines.size}): $lines")
                    lines.joinToString("\n")
                } else {
                    result.simpleText.trim()
                }
            },
            onFailure = { e ->

...
object ImageProcessor {
    ...
    /**
     * 缩放图片（保持宽高比）
     * 使用逐步降采样（每次缩小不超过50%）避免一次性大幅缩放产生摩尔纹/混叠
     * @param bitmap 原始图片
     * @param maxWidth 最大宽度
    ...
    fun scaleBitmap(bitmap: Bitmap, maxWidth: Int, maxHeight: Int): Bitmap {
        ...
        // 逐步降采样：每步最多缩小到50%，防止一次大幅缩放产生摩尔纹
        var current = bitmap
        var currentScale = 1.0f
        while (currentScale * 0.5f >= targetScale) {
            val stepWidth = (current.width * 0.5f).toInt()
            val stepHeight = (current.height * 0.5f).toInt()
            val next = Bitmap.createScaledBitmap(current, stepWidth, stepHeight, true)
        ...
        // 最后一步精确缩放到目标尺寸
        val finalScale = min(
            maxWidth.toFloat() / current.width,
            maxHeight.toFloat() / current.height
        )
        if (finalScale >= 1f) return current

        val finalWidth = (current.width * finalScale).toInt()
        val finalHeight = (current.height * finalScale).toInt()
        val result = Bitmap.createScaledBitmap(current, finalWidth, finalHeight, true)
        if (current !== bitmap) current.recycle()
        return result
    }

    /**
     * 为预览显示生成缩略图，避免直接在 UI 中渲染超大 Bitmap 导致摩尔纹
     * @param bitmap 原始图片
     * @param maxPx  预览区域最大边长（px）
     */
    fun createPreviewBitmap(bitmap: Bitmap, maxPx: Int = 1080): Bitmap {
        return scaleBitmap(bitmap, maxPx, maxPx)
    }

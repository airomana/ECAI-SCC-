...
import com.eldercare.ai.network.DiaryEntryDto
import com.eldercare.ai.utils.createWeeklyReportPerformanceTracker
...
class ConversationManager private constructor(
    ...
    private val api: BackendApi? by lazy {
        try { ApiClient.create(context, BackendApi::class.java) } catch (e: Exception) { null }
    }
    ...
    suspend fun endSession(sessionId: Long) = withContext(Dispatchers.IO) {
        ...
        // 同步更新今日情绪日志
        val newLog = updateTodayEmotionLog()
        ...
        // 上传到后端（仅父母端，有 token 时）
        val token = settingsManager.getJwtToken()
        if (token.isNotBlank() && userMessages.isNotEmpty()) {
            try {
                val combinedContent = userMessages.joinToString("；") { it.content.take(30) }
                val aiResponse = messages.lastOrNull { it.role == "assistant" }?.content ?: ""
                ...
                api?.syncDiaries(listOf(dto))
                Log.d(TAG, "Diary synced to backend")
                
                // 同时同步 EmotionLog 到后端
                if (newLog != null) {
                    val emotionLogDto = com.eldercare.ai.network.EmotionLogDto(
                        dayTimestamp = newLog.dayTimestamp,
                        dominantEmotion = newLog.dominantEmotion,
                        emotionDistributionJson = newLog.emotionDistributionJson,
                        conversationCount = newLog.conversationCount,
                        totalMessages = newLog.totalMessages,
                        summary = newLog.summary,
                        sentToFamily = newLog.sentToFamily,
                        createdAt = newLog.createdAt
                    )
                    api?.syncEmotionLog(emotionLogDto)
                    Log.d(TAG, "EmotionLog synced to backend")
                }
            } catch (e: Exception) {
                Log.w(TAG, "Diary sync failed (will retry next time)", e)
            }
        }
    }
    ...
    suspend fun generateReply(
        ...
        // 读取健康档案和个人情况
        val healthProfile = try { db.healthProfileDao().getOnce() } catch (_: Exception) { null }
        val personalSituation = try { db.personalSituationDao().getOnce() } catch (_: Exception) { null }
    ...
    /** 更新今日情绪日志 */
    private suspend fun updateTodayEmotionLog(): EmotionLogEntity {
    ...
    suspend fun generateWeeklyReport(healthProfileName: String? = null): String =
        withContext(Dispatchers.IO) {
            val performanceTracker = createWeeklyReportPerformanceTracker()
            ...
            if (entries.isEmpty() && logs.isEmpty()) {
                performanceTracker.logTotal("周报生成耗时（无数据返回）")
            ...
            performanceTracker.logDuration("调用大模型生成周报", llmStart)

            if (!llmReport.isNullOrBlank()) {
                performanceTracker.logTotal("周报生成总耗时（LLM模式）")
                return@withContext llmReport
            }

            // 本地降级生成
            val localStart = performanceTracker.mark()
            val localReport = buildLocalWeeklyReport(logs, entries, healthProfileName)
            performanceTracker.logDuration("本地降级生成周报", localStart)
            
            performanceTracker.logTotal("周报生成总耗时（本地降级模式）")
            
            return@withContext localReport
        }
    ...
    private fun buildLocalWeeklyReport(
        ...
        sb.appendLine()
        sb.appendLine("共记录 ${totalConversations} 条对话，请继续保持关爱。")

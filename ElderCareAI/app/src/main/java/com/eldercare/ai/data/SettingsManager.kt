...
class SettingsManager(context: Context) {
    ...
    companion object {
        ...
        private const val KEY_SERVER_USER_ID = "server_user_id"  // 服务端用户ID
    ...
    fun logout() {
        ...
        prefs.edit().remove(KEY_JWT_TOKEN).remove(KEY_SERVER_USER_ID).apply()
    }

    // JWT Token
    fun getJwtToken(): String = prefs.getString(KEY_JWT_TOKEN, "") ?: ""
    fun setJwtToken(token: String) = prefs.edit().putString(KEY_JWT_TOKEN, token).apply()

    // 服务端用户ID（与本地Room ID分开存）
    fun getServerUserId(): Long? {
        val id = prefs.getLong(KEY_SERVER_USER_ID, -1L)
        return if (id == -1L) null else id
    }
    fun setServerUserId(id: Long) = prefs.edit().putLong(KEY_SERVER_USER_ID, id).apply()

    fun isServerLinked(): Boolean = getJwtToken().isNotBlank()
}

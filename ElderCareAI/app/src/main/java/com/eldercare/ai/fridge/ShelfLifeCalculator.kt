...
class FridgeFoodRag(private val context: Context) {
    ...
    /**
     * 为指定食材查找规则摘要，供 RAG 增强 prompt 使用
     * 返回格式：冷藏保质期 X~Y 天，变质特征：...，建议：...
     */
    fun findRuleSummary(foodName: String, category: String, storage: String = "冷藏"): String? {
        ...
        return buildString {
            val min = rule.shelfLifeMinDays
            val max = rule.shelfLifeMaxDays
            when {
                min != null && max != null -> append("冷藏保质期 $min~$max 天")
                max != null -> append("冷藏最多放 $max 天")
                min != null -> append("冷藏至少可放 $min 天")
            }
            if (rule.spoilSigns.isNotBlank()) append("，变质特征：${rule.spoilSigns}")
            if (rule.mustDiscardIf.isNotBlank()) append("，必须丢弃：${rule.mustDiscardIf}")
            if (rule.safeFallbackAdvice.isNotBlank()) append("，建议：${rule.safeFallbackAdvice}")

...
class LlmService private constructor(private val context: Context) {
    ...
    data class DishEvaluationResult(
        val riskLevel: com.eldercare.ai.ui.screens.menu.RiskLevel,
        val advice: String
    )

    /**
     * 结合健康档案 + RAG 知识库，为某道菜生成风险评估和大白话健康建议
     */
    suspend fun evaluateDishRisk(
        dishName: String,
        healthProfile: HealthProfile?,
        ragRecords: List<RagRecord>
    ): DishEvaluationResult? = withContext(Dispatchers.IO) {
        try {
            if (!config.isConfigured() || !config.isEnabled(context)) return@withContext null

            val prompt = buildEvaluationPrompt(dishName, healthProfile, ragRecords) + 
                "\n\n请严格返回JSON格式，包含两个字段：\n1. \"riskLevel\": 风险等级，必须是 \"LOW\"、\"MEDIUM\" 或 \"HIGH\" 之一。\n2. \"advice\": 给老人的大白话建议（40-80字）。\n不要输出任何Markdown标记或多余文字。"
                    ...
                    messages = listOf(
                        DashScopeMessage(
                            ...
                            content = """
                                ...
                                规则：
                                1. 必须结合老人的具体疾病（如糖尿病、高血压等）和菜品的具体做法/成分来评估风险。即使是普通家常菜，如果含有对老人疾病不利的成分，也要相应提高风险等级。
                                2. 如果权威知识或疾病信息里提示"应避免""不推荐"，你必须严格遵守，不得放宽，风险等级必须为HIGH。
                                3. 如果你自己的知识和权威知识冲突，一律以权威知识和疾病限制为准。
                                4. 如果不确定安全性，也要偏保守，建议少吃或换成更清淡的菜。
                                5. 建议输出必须用简体中文、口语化、适合朗读，让老年人听得懂。
                                6. 必须严格返回纯JSON格式，禁止输出任何其他文字。
                            """.trimIndent()
                        ),
                        DashScopeMessage(role = "user", content = prompt)
                    )
            ...
            if (response.isSuccessful) {
                ...
                if (content != null) {
                    val text = (content as String).trim().removePrefix("```json").removePrefix("```").removeSuffix("```").trim()
                    Log.d(TAG, "风险评估生成成功: $text")
                    try {
                        val jsonObject = org.json.JSONObject(text)
                        val riskLevelStr = jsonObject.optString("riskLevel", "MEDIUM")
                        val advice = jsonObject.optString("advice", "暂时无法生成建议")
                        val riskLevel = when (riskLevelStr.uppercase()) {
                            "HIGH" -> com.eldercare.ai.ui.screens.menu.RiskLevel.HIGH
                            "LOW" -> com.eldercare.ai.ui.screens.menu.RiskLevel.LOW
                            else -> com.eldercare.ai.ui.screens.menu.RiskLevel.MEDIUM
                        }
                        return@withContext DishEvaluationResult(riskLevel, advice)
                    } catch (e: Exception) {
                        Log.e(TAG, "解析JSON失败: $text", e)
                        return@withContext null
                    }
                }
                Log.w(TAG, "风险评估响应体为空或格式不正确")
                return@withContext null
            } else {
                val errorBody = response.errorBody()?.string()
                Log.e(TAG, "风险评估 API 调用失败: ${response.code()}, $errorBody")
                return@withContext null
            }
        } catch (e: Exception) {
            Log.e(TAG, "生成风险评估时出错", e)
            return@withContext null
        }
    }
    ...
    /**
     * 第二步：结合 RAG 规则对不确定食材重新判断新鲜度和保质期
     * rulesText 格式：每行一条食材规则，由调用方从 CSV 规则库拼装
     */
    suspend fun analyzeImageWithRules(
        imageBase64: String,
        foodsWithRules: List<Pair<String, String>>,  // Pair<食材名, 规则摘要>
        visionModel: String = "qwen-vl-max"
    ): String? = withContext(Dispatchers.IO) {
        if (foodsWithRules.isEmpty()) return@withContext null
        try {
            if (!config.isConfigured() || !config.isEnabled(context)) return@withContext null

            val rulesBlock = foodsWithRules.joinToString("\n") { (name, rule) ->
                "- $name：$rule"
            }
            val namesBlock = foodsWithRules.joinToString("、") { it.first }
            ...
            val request = DashScopeMultimodalRequest(
                    ...
                    messages = listOf(
                        DashScopeMultimodalMessage(
                            ...
                            content = listOf(
                                ...
                                mapOf(
                                    "text" to buildString {
                                        append("图中有以下食材需要重新判断：$namesBlock。")
                                        append("\n\n以下是每种食材的专业保质期参考规则（冷藏条件）：")
                                        append("\n$rulesBlock")
                                        append("\n\n请结合图中食材的实际外观（颜色、形态、是否切开/开封/熟食等）和上面的规则，")
                                        append("重新判断每种食材的新鲜度和还能放几天。")
                                        append("\n必须严格只输出JSON数组（不要Markdown、不要多余文字）。格式：")
                                        append(
                                            "\n[\n" +
                                            "  {\n" +
                                            "    \"name\": \"食材名\",\n" +
                                            ...
                                            "    \"days_left\": 3,\n" +
                                            "    \"spoil_signs_observed\": [],\n" +
                                            "    \"notes\": \"判断依据（20字以内）\"\n" +
                                            "  }\n" +
                                            "]\n"
                                        )
                                        append("\n规则：")
                                        append("\n1) days_left 结合规则上限和实际外观取保守值，疑似变质填0，看不清填null")
                                        append("\n2) 只有观察到 发霉/渗液/腐烂/黑斑/软烂/发黏 才允许 freshness=疑似变质")
                                        append("\n3) 安全优先，不确定就填 freshness=未知，days_left=null")
                                    }
                                )
                            )
                        )
                ...
                parameters = DashScopeParameters(temperature = 0.0, max_tokens = 800, top_p = 0.1)
            )
        ...
        catch (e: java.net.UnknownHostException) { throw e }
        catch (e: java.io.IOException) { throw e }
        catch (e: Exception) {
            Log.e(TAG, "analyzeImageWithRules 出错", e)
            return@withContext null
        }
    }
                    ...
                    messages = listOf(
                        DashScopeMultimodalMessage(
                            ...
                            content = listOf(
                                ...
                                mapOf(
                                    "text" to buildString {
                                        append("你在帮一位老人整理冰箱食材。请看图识别食材，根据外观判断当前新鲜程度，并估算冷藏条件下还能放几天。")
                                        ...
                                        append("\n规则：")
                                        append("\n1) days_left 是根据食材当前外观和形态（整个/切开/熟食/已开封）估算冷藏下还能放的天数，疑似变质填0，看不清填null")
                                        append("\n2) spoil_signs_observed 只能从以下词里选（可为空数组）：发霉|渗液|腐烂|黑斑|变色|发黏|出水|虫|破损|皱缩|软烂")
                                        append("\n3) 如果看不清：clarity=看不清，confidence<=0.3，freshness=未知，days_left=null，spoil_signs_observed=[]")
                                        append("\n4) 只有在 spoil_signs_observed 包含 发霉/渗液/腐烂/黑斑/软烂/发黏 时，才允许 freshness=疑似变质；否则不要随便判疑似变质")
                                        append("\n5) 如果只是轻微皱缩/出水/变色但不严重：freshness=快坏 或 一般，并写 notes 说明依据")
                                        append("\n6) 安全优先：不确定就判 freshness=未知，days_left=null")
                                    }
                                )
                            )
                        )
    ...
    private fun buildEvaluationPrompt(
        ...
        return buildString {
            ...
            append("【输出要求】\n")
            append("1. 必须返回严格的JSON格式。\n")
            append("2. advice 字段只输出一小段大白话，用第二人称'您'，直接对老人说话。\n")
            append("3. 如果权威知识或疾病信息提示高风险，advice 中明确说'最好不要吃'，并给一个简单替代建议。\n")
            append("4. 如果没有把握，也要说明'为了保险起见，建议少吃或不吃'。\n")
        }
    }

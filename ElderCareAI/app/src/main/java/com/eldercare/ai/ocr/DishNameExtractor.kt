...
object DishNameExtractor {
    ...
    fun extractDishCandidates(text: String): List<DishCandidate> {
        ...
        // 如果绝大多数行都是单字，说明 OCR 库返回了字符级换行，需要合并
        // 策略：把连续的单字行合并，直到遇到多字行或行尾
        val mergedLines = mergeCharLines(rawLines)
        
        val candidates = mutableListOf<DishCandidate>()
        
        mergedLines.forEachIndexed { index, line ->
...
/**
 * 合并字符级换行：当 OCR 库把每个汉字单独换行时，将连续的单字行重新拼合成词
 * 判断依据：若超过一半的行是单字（含单个汉字或单个标点），则认为是字符级换行
 */
private fun mergeCharLines(lines: List<String>): List<String> {
    if (lines.isEmpty()) return lines

    val singleCharCount = lines.count { it.length == 1 }
    val isCharLevel = singleCharCount > lines.size / 2

    if (!isCharLevel) {
        Log.d("DishNameExtractor", "行级别正常，无需合并，行数=${lines.size}")
        return lines
    }

    Log.d("DishNameExtractor", "检测到字符级换行（单字行 $singleCharCount/${lines.size}），开始合并")
    ...
    Log.d("DishNameExtractor", "合并后行数=${result.size}，内容=$result")
    return result
}
